<!-- Bootstrap Modal -->
<div class="modal fade" id="{{ isset($package) ? 'editPackageModal' . $package->id : 'createPackageModal' }}" tabindex="-1"
                ...
                <form
                    ...
                    <div class="mb-3 row">
                        <div class="col-md-6">
                            ...
                            <select name="package_type" id="packageSelect{{ isset($package) ? $package->id : '' }}"
                                class="form-select package-select"
                                data-target="{{ isset($package) ? $package->id : '' }}" required>
                                <option value="">-- Select Package --</option>
                                <option value="basic"
                                    {{ isset($package) && $package->package_type == 'basic' ? 'selected' : '' }}>Basic
                                </option>
                                <option value="standard"
                                    {{ isset($package) && $package->package_type == 'standard' ? 'selected' : '' }}>
                                    Standard</option>
                                <option value="premium"
                                    {{ isset($package) && $package->package_type == 'premium' ? 'selected' : '' }}>
                                    Premium</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="billing_cycle" class="form-select" required>
                                <option value="Monthly"
                                    {{ isset($package) && $package->billing_cycle == 'Monthly' ? 'selected' : '' }}>
                                    Monthly</option>
                                <option value="Quarterly"
                                    {{ isset($package) && $package->billing_cycle == 'Quarterly' ? 'selected' : '' }}>
                                    Quarterly</option>
                                <option value="Yearly"
                                    {{ isset($package) && $package->billing_cycle == 'Yearly' ? 'selected' : '' }}>
                                    Yearly</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="currency" class="form-select" required>
                                <option value="INR"
                                    {{ isset($package) && $package->currency == 'INR' ? 'selected' : '' }}>INR</option>
                                <option value="USD"
                                    {{ isset($package) && $package->currency == 'USD' ? 'selected' : '' }}>USD</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select">
                            <option value="active"
                                {{ isset($package) && $package->status == 'active' ? 'selected' : '' }}>Active</option>
                            <option value="inactive"
                                {{ isset($package) && $package->status == 'inactive' ? 'selected' : '' }}>Inactive
                            </option>
                        </select>
                    </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        @if (isset($package) && $package->features)
            @php
                ...
                $features = is_string($rawFeatures) ? json_decode($rawFeatures, true) : $rawFeatures;
